...
class sensor(threading.Thread):
    ...
    def run(self):
        ...
        while self.cycling:
            ...
            #comment this to cycling as fast as possible
    ...
    def getAngleAcc(self):
        #TODO
        g=0.980665
        g=0 #TEST with new acc normalization
        pi=3.141592
        #ATTENTION atan2(y,x) while in excel is atan2(x,y)
        r=math.atan2(self.y_acc,self.z_acc+g)*180/pi
        p=math.atan2(self.x_acc,self.z_acc+g)*180/pi
        #Note that yaw value is not calculable using acc info
        # it returns a value just for keep a consistent structure
        y=0
